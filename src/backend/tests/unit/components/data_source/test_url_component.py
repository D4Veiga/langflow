...
class TestURLComponent(ComponentTestBaseWithoutClient):
    ...
    def test_url_component_proxy_forces_sync_mode(self, mock_logger, mock_recursive_loader_class):
        ...
        component.set_attributes(
            {
                "urls": ["https://example.com"],
                "use_async": True,  # User requested async
                "max_depth": 1,
                "timeout": 30,
                "format": "Text",
                "prevent_outside": True,
        ...
        # The core of the fix: use_async MUST be forced to False
        assert called_kwargs["use_async"] is False

        # Verify the user was warned
